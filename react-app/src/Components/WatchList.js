...
export default function WatchList() {
  ...
  useEffect(() => {
    ...
    .then((res)=>{
      ...
      setWatchList(res.data.data.moviesToWatch);
    })
    .catch((err)=>{
      console.log(err);
    })
    ...
    <div style={{height: window.innerHeight}}>
        <h1 style={{color:"white"}}>WatchList</h1>
          <div className="watchlist-movie-grid">
            {watchList && watchList.slice(firstIndex, secondIndex).map((movie, i)=>{
            return(
                <Card onClick={(event) => {
                ...
                className="watchlist-movie-body" 
                sx={{ 
                  width: "90%", 
                  cursor: "pointer", 
            ...
            {
            <Pagination 
                count={watchList && Math.ceil(watchList.length/itemsPerPage)} 
                variant="outlined" 
                color="secondary" 
